...
This bug report was automatically generated by OrbisHub Desktop.
        ...
        const queueResult = await pool.request()
            .input('profileId', sql.NVarChar, profileId)
            .input('toEmail', sql.NVarChar, 'bugreport@orbishub.net')
            .input('toName', sql.NVarChar, 'OrbisHub Bug Reports')
        ...
        try {
            ...
            const mailOptions = {
                from: `"${profile.fromName}" <${profile.fromEmail}>`,
                to: `"OrbisHub Bug Reports" <bugreport@orbishub.net>`,
...
ipcMain.handle('user-profile:get', async (event, userId) => {
    try {
        const pool = await getDbPool();
        if (!pool) {
            throw new Error('Database connection not available');
...
ipcMain.handle('user-profile:get-activity', async (event, userId, limit = 10) => {
    try {
        const pool = await getDbPool();
        if (!pool) {
            throw new Error('Database connection not available');
        ...
        const result = await pool.request()
            ...
            .query(`
                SELECT TOP (@limit) 
                    action, entityType, entityName, timestamp, ip, details
                FROM AuditLog 
        ...
        const activities = result.recordset.map(row => ({
            type: row.action.includes('login') ? 'login' : 
                  row.action.includes('create') ? 'create' :
                  row.action.includes('update') ? 'update' :
                  row.action.includes('delete') ? 'delete' : 'view',
            action: `${row.action} ${row.entityType}`,
            details: row.entityName,
...
// Get user statistics
ipcMain.handle('user-profile:get-stats', async (event, userId) => {
    try {
        const pool = await getDbPool();
        if (!pool) {
            throw new Error('Database connection not available');
        ...
        const userResult = await pool.request()
            .input('userId', sql.NVarChar, userId)
            .query('SELECT username, lastLogin, created_at FROM Users WHERE id = @userId');

        if (!userResult.recordset || userResult.recordset.length === 0) {
            return { 
                success: false, 
                error: 'User not found',
                data: { totalLogins: 0, totalActions: 0, lastLogin: null, accountAge: 0 }
            };
        }

        const username = userResult.recordset[0].username;
        const lastLogin = userResult.recordset[0].lastLogin;
        ...
        const loginResult = await pool.request()
            ...
            .query(`
                ...
                FROM AuditLog 
                WHERE username = @username AND action LIKE '%login%'
            `);
        ...
        const stats = {
            totalLogins: loginResult.recordset[0].count || 0,
            totalActions: actionsResult.recordset[0].count || 0,
            lastLogin: lastLogin,
            accountAge: 0 // Calculated on frontend using created_at
        };
...
ipcMain.handle('user-profile:get-permissions', async (event, userId) => {
    try {
        const pool = await getDbPool();
        if (!pool) {
            throw new Error('Database connection not available');
        ...
        // Get permissions through UserRoles and RolePermissions
        const result = await pool.request()
            .input('userId', sql.NVarChar, userId)
            .query(`
                SELECT DISTINCT 
        ...
        // Format permissions with proper categories
        const permissionList = result.recordset.map(perm => ({
            name: perm.description || perm.permission,
            category: perm.category || (perm.resource ? perm.resource.charAt(0).toUpperCase() + perm.resource.slice(1) : 'Other'),
            description: perm.description || `Can ${perm.action || 'perform actions'} on ${perm.resource || 'resources'}`
        }));

        return { success: true, data: permissionList };
    } catch (error) {
...
// Update user profile
ipcMain.handle('user-profile:update', async (event, userId, updates) => {
    try {
        const pool = await getDbPool();
        if (!pool) {
            throw new Error('Database connection not available');
        ...
        const allowedFields = ['name', 'email', 'position', 'squad'];
        const updateFields = [];
