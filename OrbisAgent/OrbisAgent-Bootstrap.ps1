...
if (-not $CoreServiceUrl) {
    # If running from downloaded script, try to detect from current URL
    if ($MyInvocation.MyCommand.Path) {
        Write-Host "Detecting Core Service URL..." -ForegroundColor Yellow
        # Try localhost first
        $CoreServiceUrl = "http://127.0.0.1:5000"
    } else {
        # Running via irm | iex, ask user
        $CoreServiceUrl = Read-Host "Enter Core Service URL (e.g., http://192.168.1.100:5000)"
    }
}
